	...
	for i, pred := range instr.Block().Preds {
		if pred.Index == b.Pred {
			...
			if !ok {
				...
				infer.Logger.Printf(f.Sprintf(PhiSymbol+"%s = %s, selected const from block %d", f.locals[instr], instr.String(), edge))
				return edge
			}
			f.locals[instr], edge = e, pred.Index
			infer.Logger.Printf(f.Sprintf(PhiSymbol+"%s = %s, selected from block %d", f.locals[instr], instr.String(), edge))
			return edge
		}
	}
